...
import json
import time
# Add shared progress storage for multi-worker compatibility
import tempfile
PROGRESS_DIR = os.path.join(tempfile.gettempdir(), "skillfinesse_upload_progress")
os.makedirs(PROGRESS_DIR, exist_ok=True)

def set_upload_progress(upload_id, progress_data):
    """Set upload progress in shared storage"""
    try:
        progress_file = os.path.join(PROGRESS_DIR, f"{upload_id}.json")
...
def get_upload_progress(upload_id):
    """Get upload progress from shared storage"""
    try:
        progress_file = os.path.join(PROGRESS_DIR, f"{upload_id}.json")
...
def serve_static(filename):
    ...
    # For all other static files, try CDN first with fallback to local
    local_path = os.path.join(LOCAL_STATIC_FOLDER, filename)
    ...
    # CDN doesn't have file or is unreachable, try local
    if os.path.exists(local_path):
        return send_from_directory(LOCAL_STATIC_FOLDER, filename)
    
    # Neither CDN nor local has file, redirect to CDN anyway (for cached/future files)
            ...
            def upload_worker():
                ...
                with app.app_context():
                    try:
                        ...
                        import time
                        progress = get_upload_progress(upload_id) or {}; progress['status'] = 'uploading'; set_upload_progress(upload_id, progress)
                        
                        # Start real upload immediately - no fake simulation
                        start_time = (get_upload_progress(upload_id) or {}).get('start_time', time.time())
                        total_size_mb = (get_upload_progress(upload_id) or {}).get('total_size_mb', 0)
                        
                        # Update to show upload starting
                        progress = get_upload_progress(upload_id) or {}
                        progress.update({
                            'percent': 1,
                            'uploaded_mb': 0,
                            'speed_mbps': 0,
                        ...
                        set_upload_progress(upload_id, progress)
                        ...
                        def progress_callback(uploaded_bytes, total_bytes):
                            if upload_id in upload_progress_store:
                                ...
                                progress = get_upload_progress(upload_id) or {}; progress.update({
                        ...
                        progress = get_upload_progress(upload_id) or {}; progress['continue_progress'] = True; set_upload_progress(upload_id, progress)
                        ...
                        if upload_result['success']:
                            ...
                            progress = get_upload_progress(upload_id) or {}; progress['continue_progress'] = False; set_upload_progress(upload_id, progress)
                            
                            # Get current progress from simulation
                            current_percent = (get_upload_progress(upload_id) or {}).get('percent', 90)
                            ...
                            # Progress to 90%
                            progress = get_upload_progress(upload_id) or {}; progress['percent'] = 90; set_upload_progress(upload_id, progress)
                            ...
                            # Progress to 95%
                            progress = get_upload_progress(upload_id) or {}; progress['percent'] = 95; set_upload_progress(upload_id, progress)
                            ...
                            progress = get_upload_progress(upload_id) or {}; progress['status'] = 'failed'; set_upload_progress(upload_id, progress)
                            progress = get_upload_progress(upload_id) or {}; progress['error'] = upload_result['error']; set_upload_progress(upload_id, progress)
                            print(f"❌ Upload failed: {upload_result['error']}")
                            
                    except Exception as e:
                        progress = get_upload_progress(upload_id) or {}; progress['status'] = 'failed'; set_upload_progress(upload_id, progress)
                        progress = get_upload_progress(upload_id) or {}; progress['error'] = str(e); set_upload_progress(upload_id, progress)
