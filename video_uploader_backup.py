        ...
        def read(self, size=-1):
            ...
            if data:
                ...
                if self.bytes_uploaded - self.last_update >= 1024*1024 or self.bytes_uploaded >= self.file_size:
                    ...
                    message = f"Uploading: {uploaded_str} / {total_str} ({percent:.1f}%)"
                    
                    self.uploader.update_progress(
                        self.upload_id, 
                        15 + (percent * 0.75),  # Scale to 15-90% range
                        message,
                        speed,
                        "uploading",
                        eta,
        ...
        try:
            ...
            if upload_id:
                with self.ProgressFileWrapper(file_path, upload_id, self) as file_wrapper:
                    response = requests.put(upload_url, headers=headers, data=file_wrapper, timeout=600)
            else:
                with open(file_path, 'rb') as file:
                    response = requests.put(upload_url, headers=headers, data=file, timeout=600)
        ...
        def upload_worker():
            with app.app_context():
                try:
                    ...
                    # CDN upload will update progress from 10% to 90%
